    ...
    if (e.encoding === "x") {
      ...
      if (r = e.items.map((l) => render(l, {
        ...
        for (let u = 0; u < l[0].children.length; u++) {
          const d = min(l.map((p) => p.children[u].children[0]).map((p) => p.x)), h = max(l.map((p) => p.children[u].children[0]).map((p) => p.x)), f = l[0].children[u].y + l[0].children[u].height / 2 + 3, o = line(d + 5, f, h + 5, f, {
            class: [
              "connection-path",
    ...
    return setup_drag(e.name, {
      ...
      on_drag: async (m, _, x) => {
        ...
        if (e.name_ghost.style.top = `${e.name.getBoundingClientRect().top - 3}px`, e.name_ghost.style.left = `${n}px`, e.name_ghost.style.width = `${l}px`, (h !== S || f !== E) && Math.abs(x.x - o) > DRAG_THRESHOLD) {
          ...
          o = x.x, e.frag.style.width = t.layout.width + "px";
        }
      },
      on_release_drag: (m) => {
        const _ = m.target.classList;
